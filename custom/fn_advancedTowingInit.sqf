...
SA_Attach_Tow_Ropes_Action = {
	...
	if([_vehicle,_cargo] call SA_Can_Attach_Tow_Ropes) then {
		...
		_canBeTowed = true;
		// Enabled towing locked vehicles 170807
		if!(missionNamespace getVariable ["SA_TOW_LOCKED_VEHICLES_ENABLED",true]) then {
			if( locked _cargo > 1 ) then {
				["Cannot attach tow ropes to locked vehicle",false] call SA_Hint;
				_canBeTowed = false;
			};
		};
		// Enabled towing in safe zones 170807
		if!(missionNamespace getVariable ["SA_TOW_IN_EXILE_SAFEZONE_ENABLED",true]) then {
